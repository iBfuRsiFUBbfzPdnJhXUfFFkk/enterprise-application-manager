...
@login_required
def proposal_export_pdf_view(request: HttpRequest) -> HttpResponse:
    """Export proposal to PDF with embedded attachments."""
    try:
        ...
        safe_name = safe_name.replace(" ", "_").lower()
        timestamp = now.strftime("%Y-%m-%d__%I:%M:%p").lower().replace(" ", "")
        filename = f"{proposal.document_id}_{safe_name}__{timestamp}.pdf"
