...
def estimation_item_move_to_bottom_view(request: HttpRequest, item_id: int) -> HttpResponse:
    """Move an estimation item to the bottom (last position)."""
    try:
        ...
        # Resequence: set other items first, then this item at the end
        with transaction.atomic():
            for index, other_item in enumerate(other_items, start=1):
                other_item.order = index
                other_item.save(update_fields=['order'])

            # Set this item to be after all others
            item.order = len(other_items) + 1
