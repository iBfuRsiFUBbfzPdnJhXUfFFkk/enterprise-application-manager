...
def add_formatted_text(paragraph, text, font_size=10):
    """
    Add text with inline markdown formatting (bold, italic, and code) to a paragraph.
    Supports **bold**, *italic*, and `code` markdown syntax.
    """
    # Process code first (backticks), then bold, then italic
    code_pattern = r'(`[^`]+?`)'
    code_parts = re.split(code_pattern, text)
    ...
    for code_part in code_parts:
        if code_part.startswith('`') and code_part.endswith('`') and len(code_part) > 2:
            ...
            run.font.highlight_color = 7  # Light gray background (WD_COLOR_INDEX.GRAY_25)
        else:
            # Process bold in non-code parts
            bold_pattern = r'(\*\*[^*]+?\*\*)'
            bold_parts = re.split(bold_pattern, code_part)
            ...
            for bold_part in bold_parts:
                if bold_part.startswith('**') and bold_part.endswith('**') and len(bold_part) > 4:
                    ...
                    italic_pattern = r'(\*[^*]+?\*)'
                    italic_parts = re.split(italic_pattern, bold_part)
...
def estimation_export_docx_view(request: HttpRequest, model_id: int) -> HttpResponse:
    ...
    # Generate current timestamp for when document was created
    generated_at = datetime.now(timezone.utc)

    # Get the latest history entry to find last modification time
    last_modified = None
    if hasattr(estimation, 'history') and estimation.history.exists():
        latest_history = estimation.history.first()
        if latest_history:
            last_modified = latest_history.history_date

    # Create metadata table
    meta_info_table = document.add_table(rows=3, cols=2)
    meta_info_table.style = 'Light Grid Accent 1'

    # Generated by
    row = meta_info_table.rows[0]
    row.cells[0].text = 'Generated By'
    user_display = str(request.user) if request.user.is_authenticated else 'Anonymous'
    row.cells[1].text = user_display

    # Generated at
    row = meta_info_table.rows[1]
    row.cells[0].text = 'Document Generated'
    generated_str = generated_at.strftime('%B %d, %Y at %I:%M %p UTC')
    row.cells[1].text = generated_str

    # Last modified
    row = meta_info_table.rows[2]
    row.cells[0].text = 'Last Modified'
    if last_modified:
        modified_str = last_modified.strftime('%B %d, %Y at %I:%M %p UTC')
        row.cells[1].text = modified_str
    else:
        row.cells[1].text = 'Not available'

    # Make all text in metadata table smaller
    for row in meta_info_table.rows:
    ...
    if estimation.description:
        desc_para = document.add_paragraph(estimation.description)
