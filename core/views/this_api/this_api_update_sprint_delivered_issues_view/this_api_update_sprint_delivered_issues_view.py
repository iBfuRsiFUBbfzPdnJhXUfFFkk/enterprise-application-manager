...
from core.views.this_api.this_api_update_sprint_delivered_issues_view.calculate_delivered_issues import \
    calculate_delivered_issues
from kpi.models.key_performance_indicator_sprint import KeyPerformanceIndicatorSprint


def this_api_update_sprint_delivered_issues_view(request: HttpRequest) -> HttpResponse:
    ...
    for sprint in sprints:
        ...
        delivered_issues, project_mapping = calculate_delivered_issues(all_group_issues=all_group_issues)
        updated_count = 0
        for gitlab_username, data in delivered_issues.items():
            issue_count = data["count"]
            total_weight = data["weight"]
            context_switching = len(project_mapping.get(gitlab_username, []))  # Unique project count
            ...
            try:
                ...
                kpi.number_of_story_points_delivered = total_weight
                kpi.number_of_context_switches = context_switching  # Update context switching metric
                kpi.save()
                updated_count += 1
