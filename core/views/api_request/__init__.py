from core.views.api_request.api_request_add_view import api_request_add_view
from core.views.api_request.api_request_delete_view import (
    api_request_delete_view,
)
from core.views.api_request.api_request_detail_view import (
    api_request_detail_view,
)
from core.views.api_request.api_request_edit_view import api_request_edit_view
from core.views.api_request.api_request_view import api_request_view
