...
function Show-DatabaseManagement {
    ...
    Write-Host "  9) Optimize Django history (one-time operation)"
    Write-Host "  0) Back"
    Write-Host "  q) Exit script"
    Write-Host ""
    Write-Host -NoNewline "Enter choice: "
    ...
    try {
        switch ($dbChoice) {
            ...
            '9' {
                Clear-Host
                Print-Header
                Print-Warning "Django History Optimization - One-Time Operation"
                Write-Host ""
                Print-Info "This will:"
                Write-Host "  1. Create database backup (JSON + SQLite file)"
                Write-Host "  2. Run migrations to drop 23 Historical* tables"
                Write-Host "  3. Run VACUUM to reclaim disk space"
                Write-Host "  4. Show database size before/after"
                Write-Host ""
                Print-Warning "Expected results:"
                Write-Host "  • 20-30% immediate database size reduction"
                Write-Host "  • 65-75% reduction in future database growth"
                Write-Host "  • No history tracking for 26 models (GitLab sync, KPI, user prefs)"
                Write-Host ""
                Print-Warning "This operation is irreversible without restoring from backup!"
                Write-Host ""
                $confirm = Read-Host "Continue? Type 'yes' to confirm"
                ...
                if ($confirm -eq "yes") {
                    ...
                    if ($LASTEXITCODE -eq 0) {
                        ...
                        Print-Warning "You can restore from: $backupSqlite"
                        Write-Host ""
                        Read-Host "Press Enter to continue"
                        return
                    }
                    ...
                    if ($LASTEXITCODE -eq 0) {
                        Print-Success "VACUUM completed successfully!"
                    } else {
                        Print-Warning "VACUUM command completed (warnings may be ignored)"
                    }
                    Write-Host ""

                    # Show database size after
                    Print-Info "Database size after optimization:"
                    if (Test-Path $dbPath) {
                        $sizeBytes = (Get-Item $dbPath).Length
                        $sizeMB = [math]::Round($sizeBytes / 1MB, 2)
                        Write-Host "  ${sizeMB} MB ($dbPath)"
                    ...
                    Print-Success "Django history optimization completed!"
                    Write-Host ""
                    Print-Info "Summary:"
                    Write-Host "  • Backups saved in: $backupDir"
                    Write-Host "  • 23 Historical* tables dropped"
                    Write-Host "  • 26 models now have history disabled"
                    Write-Host "  • Database space reclaimed via VACUUM"
                    Write-Host ""
                    Print-Warning "Note: This is a one-time operation. You can remove option 9 from the menu."
                } else {
                    Print-Info "History optimization cancelled"
                }
                Write-Host ""
                Read-Host "Press Enter to continue"
            }
