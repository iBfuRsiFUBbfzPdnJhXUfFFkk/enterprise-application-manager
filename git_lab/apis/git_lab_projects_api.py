...
def git_lab_projects_api(
    ...
    for git_lab_group in git_lab_groups:
        try:
            ...
            if group is None:
                continue
            projects: list[GroupProject] = cast(
                typ=list[GroupProject],
                val=group.projects.list(**query_parameters)
            )
    ...
    for project_dict in project_dicts:
        ...
        if project_id is None:
            continue
        get_or_create_tuple: tuple[GitLabProject, bool] = GitLabProject.objects.get_or_create(id=project_id)
        git_lab_project: GitLabProject = get_or_create_tuple[0]
